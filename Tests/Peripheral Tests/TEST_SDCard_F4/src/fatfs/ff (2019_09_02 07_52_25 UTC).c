...
BYTE wmode /* !=0: Check write protection for write access */
  ...
  *rfs = fs; /* Return pointer to the corresponding file system object */
  ...
  if (fmt == FS_FAT32) {
    ...
    if (disk_read(fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK && LD_WORD(fs->win+BS_55AA) == 0xAA55 && LD_DWORD(fs->win+FSI_LeadSig) == 0x41615252 && LD_DWORD(fs->win+FSI_StrucSig) == 0x61417272) {
